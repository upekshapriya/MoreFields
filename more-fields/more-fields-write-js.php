...
?>
		...
		?>
			...
			if ((jQuery("h2").filter(":contains('<?php _e('Add New Post'); ?>')").html()) || (jQuery("h2").filter(":contains('<?php _e('Add New Page'); ?>')").html())) { // mod
				jQuery("#menu-<?php echo sanitize_title($type); ?> li.wp-first-item").next().addClass("current");
			} else if ((jQuery("h2").filter(":contains('Posts Add New')").text()) || (jQuery("h2").filter(":contains('<?php _e('Edit Post'); ?>')").html())){
				jQuery("#menu-<?php echo sanitize_title($type); ?> li.wp-first-item").addClass("current"); //mod
				jQuery(".current > a").addClass("current");
			}
			<?php $print_type = str_replace('+', ' ', $type); ?>
			jQuery("h2").filter(":contains('<?php _e('Add New Post'); ?>')").html('<?php echo __('Add New') . ' ' . $print_type; ?>');  //mod
			jQuery("h2").filter(":contains('<?php _e('Add New Page'); ?>')").html('<?php echo __('Add New') . ' ' . $print_type; ?>'); // mod
			<?php if ($pages[$type]['plural']) {$edit_name = urlencode($pages[$type]['plural']);} else $edit_name =  $print_type . 's'; // mod ?>
			<?php $edit_name = str_replace('+', ' ', $edit_name); ?>
			jQuery("h2").filter(":contains('Posts Add New')").html('<?php echo $edit_name . '<a href="post-new.php?type=' . $print_type . '" class="button add-new-h2">Add New</a>'; ?>'); // mod
			jQuery("h2").filter(":contains('<?php _e('Edit Post'); ?>')").html('<?php echo __('Edit') . ' ' . $print_type; ?>');
