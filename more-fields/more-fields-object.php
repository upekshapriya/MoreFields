...
//				array(__('Password Protect This Post'), 'passworddiv', true),
	...
	function wp_default_scripts(&$scripts) {
		...
		if (!file_exists($src)) $src = get_option('siteurl') . '/wp-content/plugins/more-fields/post-3.5.1.js';
...
//			if ($page['name'] == 'Post' || $page['name'] == 'Page') continue;
	...
	//		$menu[21.001] = $menu[20]; //][0] = $page['name'];
			...
			if ($page['plural']) $edit_name = urlencode($page['plural']); else $edit_name =  urlencode($page['name']) . 's'; // mod
			$edit_name = str_replace('+', ' ', $edit_name);
			$submenu[$link][5] = array($edit_name, 'edit_posts', $edit_link); // mod
			$submenu[$link][10] = array(__('Add new'), 'edit_posts', $link); // mod
